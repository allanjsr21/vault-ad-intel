...
<script>
  ...
  // ===== favoritos de empresas (localStorage) =====
  const FAV_KEY = 'vault_fav_companies';
  const favGet = () => { try { return new Set(JSON.parse(localStorage.getItem(FAV_KEY) || '[]')); } catch { return new Set(); } };
  const favSave = (s) => localStorage.setItem(FAV_KEY, JSON.stringify([...s]));
  function toggleFav(name) {
    const s = favGet(); s.has(name) ? s.delete(name) : s.add(name); favSave(s); paintControls(); apply();
  }

  // STATE + RENDER
  let state = { view: 'ads', company: null, favOnly: false, filter: 'all', lang: 'all', sort: 'rank' };
  ...
  function apply() {
    ...
    if (state.view === 'companies') {
      const list = companies.filter(c => !ex.has(c.name))
    ...
    if (state.favOnly) list = list.filter(a => fav.has(a.pageName));
  ...
  function paintControls() {
    ...
    let v = vbtn('ads', '📋 Por anúncio', state.view === 'ads' && !state.company)
          ...
          + \`<button id="favtoggle" class="px-3 py-1.5 rounded-lg text-xs font-semibold transition \${state.favOnly ? 'bg-amber-400 text-black' : 'bg-white/5 text-amber-300 border border-amber-400/20 hover:bg-amber-500/15'}">⭐ Favoritas (\${favN})</button>\`;
